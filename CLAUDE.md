...
crates/
  ...
  core-win/       # windows-rs impls: IP Helper (Create/DeleteIpForwardEntry2, GetIpForwardTable2),
                  #   adapter reconfig (unicast IP, DNS), ICMP ping, adapter listing
  core-bin/       # bypass-core.exe: windows-service host + tokio named-pipe IPC server
...
- Pipe name `\\.\pipe\BypassToolCore`; line-delimited JSON-RPC 2.0, server pushes events after `SubscribeEvents`.
- Log: tracing + tracing-appender daily rolling, 7 days retention, `%ProgramData%\BypassTool\logs\`.
